...
def build_table(
    ...
    print(f"Period: [{start}, {end})  Rows: {len(out_df):,}  Cols: {len(out_df.columns)}")
    print(f"COMEPHORE kept rows: {len(comephore):,}  from {comephore['datetime'].min()} to {comephore['datetime'].max()}")
#%%
