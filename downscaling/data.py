...
def load_raw_downscaling_table(path: str | None = None, sep: str = ";") -> pd.DataFrame:
    ...
    """
    if path is None:
        from downscaling.settings import DOWNSCALING_TABLE
...
def prepare_modeling_dataframe(
    ...
    df = df.loc[df["Y_obs"].notna() & (df["Y_obs"] > 0)].copy() # keep only positive observations
...
# Modeling matrices and blocked splits
